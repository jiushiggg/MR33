...
int8_t updata_handle(uint8_t** addr, uint8_t n, rf_parse_st* info, void * extra)
{
    ...
    switch(updata_status){
        case UPDATA_START:
            ...
            buffer1 = (trans_struct *)tmp->next;
            buffer2 = (trans_struct *)tmp;
            rf_queue_init(buffer1->buf, buffer1->buf_len, buffer2->buf, buffer2->buf_len);
#if 0
